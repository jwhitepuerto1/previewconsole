  ...
  return (
    <div className="card">
      <div className="flex items-baseline justify-between">
        ...
        <p className="text-sm text-slate-500">of {fmtMoney(summary.raise_target ?? 0)} target</p>
      </div>
      ...
      <p className="mt-1 text-xs text-slate-500">{summary.percent_raised}% raised</p>

      <div className="mt-6 grid grid-cols-2 gap-4">
        <div>
          <p className="text-lg font-semibold text-slate-100">{fmtMoney(summary.soft_committed)}</p>
          <p className="text-xs text-slate-500">Soft committed ({summary.investor_count_soft} investors)</p>
        </div>
        <div>
          <p className="text-lg font-semibold text-slate-100">{fmtMoney(summary.hard_committed)}</p>
          <p className="text-xs text-slate-500">Hard committed</p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-1">
          {data.events.slice(0, 5).map((e, i) => (
            <div key={i} className="flex justify-between text-sm">
              <span className="text-slate-400">{e.event_type.replace("_", " ")}</span>
